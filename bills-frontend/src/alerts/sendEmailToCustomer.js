...
function SendEmailToCustomer(props) {
    const [messageAlert, setMessageAlert] = useState(false);

    const sendEmail = (to, subject, body) => {
        setMessageAlert(true);
        props.handleClose();
        axios.post(base_url + '/api/mail-without-bill/',
            { to, subject, body, user: props.user.avatar_id }, {
            headers: {
            ...
            if (response.status === 200) {
                props.handleMessageSnackbar(`Email sent to ${to} !`, 'success');
            }
        }).catch((error) => {
            setMessageAlert(false);
...
export default connect(mapStateToProps, null)(SendEmailToCustomer);