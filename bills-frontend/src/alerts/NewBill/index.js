...
const NewBillDialog = (props) => {
                ...
                .then(async data => {
                    ...
                    try {
                        if (data.length !== 0) {
                            ...
                            setFinalItems([...data[0]['items'].reverse()]);
    ...
    const updateItem = async (event) => {
        event.preventDefault();
        let finalItems1 = finalItems;
        var ind = await finalItems1.findIndex(value => value.no === currentItem['no']);
        let amt = await roundToTwo(Number(selectedItem['rate']) * Number(quantity));
        finalItems1[ind].quantity = quantity;
        finalItems1[ind].amount = amt;
        setFinalItems([...finalItems1]);
        setSelectedItem(null);
        setCurrentItem(null);
        setQuantity('1');
